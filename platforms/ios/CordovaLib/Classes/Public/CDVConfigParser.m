...
//        self.startPage = attributeDict[@"src"];
        if([KHSSrcPageManager loadConfigureStartPage]) {
            self.startPage = attributeDict[@"src"];
            NSLog(@"--------------加载本地配置的地址%@", self.startPage);
        } else {
            self.startPage = [KHSSrcPageManager networkStartPage];
            NSLog(@"--------------加载网络端地址%@", self.startPage);
        }
    }
}
